...
_manifest_fleet_generated_by() {
    local cli_root="${MANIFEST_CLI_CORE_MODULES_DIR%/modules}"
    local version=""
    if [[ -n "$cli_root" && -f "$cli_root/VERSION" ]]; then
        version=$(tr -d '[:space:]' < "$cli_root/VERSION" 2>/dev/null || echo "")
    fi
...
generate_start_tsv() {
    ...
    echo "# Generated by: $(_manifest_fleet_generated_by)"
