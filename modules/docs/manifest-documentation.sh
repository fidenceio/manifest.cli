...
update_readme_version() {
    ...
    # One path for every repo, this one included. There used to be a
    # canonical-repo branch here that replaced a "## 📋 Version Information"
    # section, or failing that ran `sed -e "s|\`old\`|\`new\`|g"` over the whole
    # README to chase a bare backticked version. That was a second derivation of
    # "put the version in the README" — positional where this one is delimited,
    # and it existed solely to serve this repo. Deleted: the version lives in
    # VERSION, and a reader gets it from the release, the tag, or CHANGELOG.md.
    ...
    # The managed block is the ONLY thing that authorises writing to a README
    # that already exists. There used to be a branch here that inferred machine
    # authorship from content phrases — crediting Manifest in prose, or having a
    # version table — and handed the file to create_default_readme, a whole-file
    # replacement. Those phrases are things a human writes on purpose, so the
    # inference cost people their front page (see tests/docs_readme_preserve.bats).
...
generate_docs_index() {
    ...
    # One path for every repo, this one included. There used to be a
    # canonical-repo branch here that sed-patched "**Version:**" and "**Updated:**"
    # lines in place, and wrote its own full-page template when no index existed.
    # It was a second derivation of the same fact, positional where the managed
    # blocks below are delimited, and its template was the only emitter of the
    # "<!-- Manifest CLI v -->" marker — which is why the legacy-index predicate
    # that looked for that marker could never fire on a repo Manifest manages.
    if [[ -f "$index_file" ]]; then
        # A Manifest-managed block is the ONLY thing that authorises writing to
        # an index that already exists. There used to be a third branch here
        # that inferred machine authorship from the file's title and replaced
        # the whole file; the owner's hand-written sections were the cost, and
        # the inference was never sound (see tests/docs_index_preserve.bats).
        # A file with no managed block is preserved and reported, never guessed
        # about — if it should become managed, adding the block is the owner's
        # call, and the block must be added around existing content, not by
        # overwriting it.
        if manifest_file_has_managed_block "$index_file" "$MANIFEST_CLI_INDEX_METADATA_START" "$MANIFEST_CLI_INDEX_METADATA_END" && manifest_file_has_managed_block "$index_file" "$MANIFEST_CLI_INDEX_CURRENT_RELEASE_START" "$MANIFEST_CLI_INDEX_CURRENT_RELEASE_END"; then
            local metadata_file current_release_file
            metadata_file=$(mktemp "$(manifest_make_scratch_path docs)/tmp.XXXXXXXX")
            current_release_file=$(mktemp "$(manifest_make_scratch_path docs)/tmp.XXXXXXXX")
            generate_external_index_metadata_block "$version" > "$metadata_file"
...
prepend_root_changelog_entry() {
    ...
    # Assemble beside the target, then rename into place (TRACKER §3).
...
_manifest_prune_root_changelog() {
    ...
    # already stated in manifest-install-paths.sh, applied here (TRACKER §3).
    local trimmed
