...
# Note: root CHANGELOG.md is updated by the ship orchestrator
# (manifest-orchestrator.sh, "Updating main CHANGELOG.md for GitHub" step),
# which sed-replaces version strings in user-crafted Keep-a-Changelog
# files and falls back to copying docs/CHANGELOG_v<version>.md only when
# the root file is empty or a legacy template. A previous duplicate
# implementation here was deleted to remove the divergence.
