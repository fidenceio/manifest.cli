...
set_yaml_value() {
    ...
    case "$_MANIFEST_YAML_PARSER" in
        "yq")
            yq e ".${dotpath} = \"${value}\"" -i "$yaml_file" 2>/dev/null
            if [[ $? -ne 0 ]]; then
...
write_full_yaml() {
    ...
    for yaml_path in "${!_MANIFEST_YAML_TO_ENV[@]}"; do
        ...
        env_value="${!env_var:-}"
        if [[ -n "$env_value" ]]; then
            # Escape single quotes in value for safe transport
            local escaped_value="${env_value//\'/\'\\\'\'}"
            mappings+="${yaml_path}=${escaped_value}"$'\n'
        fi
    done
...
try:
    ...
    with open('${yaml_file}', 'w') as f:
        ...
        # Dump each top-level section with a comment
