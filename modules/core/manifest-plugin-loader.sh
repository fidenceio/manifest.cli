...
MANIFEST_CLI_CLOUD_PLUGINS_DIR="${MANIFEST_CLI_CLOUD_DIR:-$HOME/.manifest-cloud}/cli-plugins"
export MANIFEST_CLI_CLOUD_PLUGINS_DIR

# Try to source a plugin file. Returns 0 if loaded, 1 if absent.
manifest_load_plugin() {
    local plugin_path="$MANIFEST_CLI_CLOUD_PLUGINS_DIR/$1"
