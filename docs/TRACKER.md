...
- **1.3 Stop the ship → tap-remote formula push from silently falling through.**
  ...
  - **Deliverable:** either (a) **preferred** — push the formula update directly to the `fidenceio/homebrew-tap` remote from any checkout (no dependency on the user's local brew state), or (b) fail the ship with a non-zero exit and a structured error when the brew-managed tap dir is absent and the push would otherwise be a silent no-op. Add a regression proving a ship without the brew-managed tap dir does not exit 0 with a stale tap remote.
