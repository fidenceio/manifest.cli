...
- **§2.8 Reconcile the remaining `validation:` knobs with their (missing) gates.**
  - **Status:** T2.
  - **Why:** the 2026-05-26 audit found the `validation:` block was mostly decorative. `require_expected_branch` and `allow_branch_operations` were fully inert and have been **removed** (no branch-workflow enforcement — that is left to the user). The other three knobs are still suspect: `require_clean_status` is declared and registered but no apply/preflight gate reads it (flipping the flag changes nothing); `enforce_dependencies` is declared + registered with no consumer; `strict` is reachable via `get_fleet_setting` short keys but isn't even in the default heredoc and has no effect (verified 2026-06-02). Config that advertises a guarantee the tool doesn't keep is a contract hole.
...
- **§5.9 PR apply-event audit coverage.**
  - **Status:** T3.
  - **Why:** the CLI apply-event audit log shipped — every apply that crosses the apply-guard `manifest_execution_require_apply` ([`manifest-execution-policy.sh`](../modules/core/manifest-execution-policy.sh) ~line 114) appends one redacted NDJSON event to `$HOME/.manifest-cli/audit/apply-events.ndjson` with actor / source (`cli` / `cli-fleet`) / command / scope / plan hash / exit status, covering `ship repo`, `ship fleet`, `prep`, and `refresh`. But `manifest pr … -y` does **not** route through that guard — [`modules/pr/manifest-pr-native.sh`](../modules/pr/manifest-pr-native.sh) calls `manifest_execution_apply_header` (lines ~93/158/197/228) and runs `gh` directly, never reaching `manifest_execution_require_apply` — so PR applies still emit no audit event. The `cli-pr` source named in the audit design is therefore not yet populated (verified 2026-06-02).
  - **Deliverable:** emit a `cli-pr` apply event for each `-y`-gated PR mutation (create / merge / ready / update). Either route PR applies through a shared emit point or call `manifest_audit_apply_event` (shipped, [`manifest-shared-utils.sh`](../modules/core/manifest-shared-utils.sh) ~line 361) directly at each PR apply boundary, setting `MANIFEST_CLI_AUDIT_SOURCE=cli-pr`. PR ops have no version plan, so the plan-hash field is empty/N/A — confirm the consumer tolerates an empty `plan_hash`. Add a regression asserting one `cli-pr` event per PR apply with the gh-backed command recorded and redaction applied.
