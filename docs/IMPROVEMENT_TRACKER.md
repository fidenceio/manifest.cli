...
- [x] **38. Configurable archive retention.** Done 2026-05-05; refactored same day after user feedback that the original `trigger`/`keep_recent` design was over-abstracted and assumed SemVer. Final shape: a single YAML key `docs.retain` (env: `MANIFEST_CLI_DOCS_RETAIN`) accepting `"N versions"`, `"N days"`, or `"off"`. Default `"10 versions"`. No version-naming assumption — "versions" just means version-tagged files in `sort -V` order; "days" uses `find -mtime` for portable mtime filtering with no new date helpers needed. `_manifest_parse_retention` in [modules/docs/manifest-cleanup-docs.sh](../modules/docs/manifest-cleanup-docs.sh) parses the spec; `main_cleanup` enumerates candidates in one pass, picks the keep-set in a second pass, archives the rest in a third. Move-log header records `Retain: <spec>`. [examples/manifest.config.yaml.example](../examples/manifest.config.yaml.example) documents the field with inline comments.
  - **Why the refactor:** the first cut had `docs.archive.trigger` (4 enum values: every_ship/minor_or_major/major_only/manual), `docs.archive.keep_recent` (integer with implicit major-vs-minor granularity from bump type), and bump-type plumbing through the orchestrator. User flagged that "scope" collided with the existing `repo|fleet` scope vocabulary and that the patch/minor/major split assumed SemVer. Collapsed to one field, two unit forms; dropped bump-type plumbing entirely.
  - **Coverage:** 11 bats cases in [tests/archive_retention.bats](../tests/archive_retention.bats) — four parse-helper cases, three versions-mode cases (`"1 version"`, `"3 versions"`, default `"10 versions"`), one off-mode case, one days-mode case using `touch -t` to set mtime 30 days back, one malformed-spec error case, one move-log-header check. Existing pre-move-safety and move-log tests updated to set `MANIFEST_CLI_DOCS_RETAIN="1 version"` so seeded files predictably archive. Full suite: 273/273.
...
**Sequencing:** ~~#37~~, ~~#38~~, ~~#39~~, ~~#40~~, and ~~#42~~ shipped 2026-05-05. ~~#41~~ declined 2026-05-05 (filesystem already covers it). Only #36 (LLM-backed release-note generation) remains — highest-leverage but blocks on a provider script being written.
