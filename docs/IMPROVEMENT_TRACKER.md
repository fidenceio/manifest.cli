...
- [ ] **42. Tighten boilerplate-stub prune to exclude doc-review-only changelogs.** Surfaced while completing #37: the 709d6a5 prune correctly removed 438 fully-empty stubs but left behind ~8 v46 archives whose only content was a "Documentation review: impact appears low (N changed files, report: ...)" line auto-emitted by [manifest-doc-review.sh](../modules/git/manifest-doc-review.sh). These archives have no user-facing substance and should never have survived.
  - **Approach:** extend the survivor-detection logic (whichever script formed the basis of 709d6a5; if not preserved, redo as a one-shot in `scripts/`) to treat changelogs whose only non-Summary section is `### Documentation` containing exclusively `Documentation review:` bullets as boilerplate. Keep the regular Documentation section when it contains real entries.
  - **Definition of done:** rerunning the prune over `docs/zArchive/v46/` removes the 8 doc-review-only archives (v46.4.0, v46.4.2, v46.6.0, v46.8.0, v46.9.0, v46.10.0, v46.11.0, v46.11.2). The corresponding `RELEASE_v46.x.md` files go too (paired). `INDEX.md` regenerates. No regression on substantive survivors.

**Sequencing:** #36 is highest-leverage but blocks on a provider script being written. ~~#37~~ shipped 2026-05-05. #38 / #39 / #40 share archival infra and naturally cluster. #41 is most useful after #38 lands (so retention metadata is meaningful in `archive list` output). #42 is unrelated to the others and quick to ship — fold in next time the archive structure is touched.
