...
- [ ] **43. Safe-by-default execution contract.** Planned breaking change. Every mutating command previews by default, `--dry-run` is the explicit preview spelling, and `-y` / `--yes` is required to apply. `--local` limits scope but does not authorize mutation by itself. `ship` is release-only and must not call PR create/check/ready/queue; PR behavior belongs only under `manifest pr ...`. Full implementation checklist: [Safe-by-Default Execution TODO](SAFE_BY_DEFAULT_EXECUTION_TODO.md).
