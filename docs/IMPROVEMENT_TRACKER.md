...
- [x] **4. Start a test harness.** bats-core adopted. Suite at `tests/` (29 tests across `yaml.bats`, `version.bats`, `canonical_repo.bats`, `safety_gate.bats`). Runner at `scripts/run-tests.sh`. Surfaced two findings:
  ...
  - **New finding:** `manifest_origin_repo_slug` is defined in **two** modules with subtly different signatures, and the canonical-repo gate previously existed as both `should_update_homebrew_for_repo` and `manifest_is_canonical_repo`. The env surface has since been collapsed to `MANIFEST_CLI_CANONICAL_REPO_SLUGS`. See tracker item #27 below.
...
- [x] **13. One deprecation format.** `log_deprecated <old> <new> [<note>]` added to `manifest-shared-utils.sh`. Wired into `manifest sync`, `manifest update`, and `manifest prep <type>`. Single-emit-per-session via `MANIFEST_CLI_DEPRECATIONS_WARNED`; suppressed by `MANIFEST_CLI_QUIET_DEPRECATIONS=1`. Four bats tests in `tests/deprecation.bats`.
...
- [x] **27. Consolidate canonical-repo detection.** Deleted the duplicate `manifest_origin_repo_slug` and `should_update_homebrew_for_repo` from `manifest-core.sh`; replaced with a one-line back-compat shim that delegates to `manifest_is_canonical_repo`. Canonical repo overrides now use only `MANIFEST_CLI_CANONICAL_REPO_SLUGS`.
...
- [x] **35. Lift remaining env-only settings into YAML.** Promoted the user-facing env-only settings to first-class YAML keys while keeping exported `MANIFEST_CLI_*` values as highest-precedence process overrides for CI and backward compatibility.
  - **High priority:**
    - [x] `release.canonical_repo_slugs` — replaces env-only `MANIFEST_CLI_CANONICAL_REPO_SLUGS`; no legacy env alias is retained.
    - [x] `ship.interactive` — today's `MANIFEST_CLI_INTERACTIVE_MODE`. Distinct from `debug.interactive`, which controls a different surface.
    - [x] `fleet.mode`, `fleet.root`, `fleet.config_filename` — today's `MANIFEST_CLI_FLEET_MODE` / `_ROOT` / `_CONFIG_FILENAME`.
  - **Medium priority:** [x] `automation.auto_confirm` (`MANIFEST_CLI_AUTO_CONFIRM`), [x] `deprecations.quiet` (`MANIFEST_CLI_QUIET_DEPRECATIONS`), [x] `network.offline` (`MANIFEST_CLI_OFFLINE_MODE`), [x] `cloud.skip` (`MANIFEST_CLI_CLOUD_SKIP`), [x] `cloud.api_key_env` (secret-reference form that hydrates `MANIFEST_CLI_CLOUD_API_KEY` from the named env var), [x] `security.private_files` (`MANIFEST_CLI_SECURITY_PRIVATE_ENV_FILES`).
  - **Schema drift fixes:** added the mapped-but-undocumented keys to `examples/manifest.config.yaml.example` — `version.regex`, `version.validation`, `debug.enabled`, `debug.verbose`, `debug.log_level`, `debug.interactive`, `pr.profile`, `pr.enforce_ready`. Wired `project.team → MANIFEST_CLI_PROJECT_TEAM`.
...
- **Double-commit guard.** Single-repo fleets (where `MANIFEST_CLI_FLEET_ROOT == service.path`) would otherwise emit two commits — fleet root pass + service pass — for the same staged tree. Helper short-circuits the service iteration when `path == root_dir`.
- **Helper extraction made it testable.** Putting the commit pass in `_refresh_fleet_commit_changes` lets bats stub `get_fleet_service_property` + `MANIFEST_CLI_FLEET_SERVICES` and exercise the matrix without booting the full fleet stack (which requires `manifest.fleet.config.yaml`, `load_fleet_config`, `_fleet_require_initialized`, etc.).
...
- **Filter mechanism: env-override of `$MANIFEST_CLI_FLEET_SERVICES`.** Both `_fleet_prep_run` and `fleet_docs_generate` iterate the env var directly, so overriding it for the duration of `fleet_ship` filters every code path that respects it — no need to thread an extra "service list" arg through five functions. The override is restored on every exit path (success or failure) via a `while :; do ... break; done` one-shot block + a single `MANIFEST_CLI_FLEET_SERVICES="$_saved_services"` cleanup line.
- **Forward `--only`/`--except` to `manifest_fleet_pr_dispatch` too.** The Cloud plugin owns its own service iteration; if it doesn't read `$MANIFEST_CLI_FLEET_SERVICES` (we can't see the plugin source from this repo), forwarding the flags lets it apply the same filter on its side. The native stub harmlessly ignores extra args ("PR feature requires Manifest Cloud").
- **Word-boundary safety: substring match with space padding, not `grep -w`.** `grep -w` treats hyphens as separators, so `--only alpha` would falsely match `alpha-svc`. Switched to `[[ " $MANIFEST_CLI_FLEET_SERVICES " == *" $name "* ]]` — only spaces are token separators, which matches how the service list is actually built.
