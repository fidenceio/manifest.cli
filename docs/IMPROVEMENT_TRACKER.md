...
- [x] **38. Configurable archive retention.** Done 2026-05-05. Two new YAML keys plumbed through [manifest-yaml.sh](../modules/core/manifest-yaml.sh) and [manifest-config.sh](../modules/core/manifest-config.sh): `docs.archive.keep_recent` (default `1`) and `docs.archive.trigger` (default `minor_or_major`, options `every_ship | minor_or_major | major_only | manual`). The orchestrator passes the bump type to `main_cleanup` so the trigger can early-return when mismatched; manual `manifest docs cleanup` calls don't pass a bump type and always run. Retention is enforced by `_manifest_archive_should_keep` in [modules/docs/manifest-cleanup-docs.sh](../modules/docs/manifest-cleanup-docs.sh); the move-log header is extended with `Sweep: trigger=...; bump=...; keep_recent=...`. [examples/manifest.config.yaml.example](../examples/manifest.config.yaml.example) documents the new keys with inline comments.
  - **Granularity convention (judgment call):** `keep_recent=N` retains the current granularity-unit + `N` previous units. Granularity is **major** for major bumps and **minor** otherwise (patch / minor / manual). Cross-major files are always archived under non-major bumps regardless of `keep_recent`. The original spec wording for major bumps was ambiguous; this linear interpretation is internally consistent and matches the spec exactly for minor bumps.
  - **Coverage:** 13 bats cases in [tests/archive_retention.bats](../tests/archive_retention.bats) — six trigger×bump-type permutations covering all four trigger options, four retention boundary cases (`keep_recent=0/1/2` minor + `keep_recent=99` cross-major guard), two major-bump retention cases (`keep_recent=0/1`), and one move-log-header check. Full suite: 275/275.
...
**Sequencing:** #36 is highest-leverage but blocks on a provider script being written. ~~#37~~, ~~#38~~, ~~#39~~, ~~#40~~, and ~~#42~~ shipped 2026-05-05. Only remaining items are #36 and #41 (`manifest docs archive list/show/restore` CLI), now unblocked since #38's retention metadata exists.
