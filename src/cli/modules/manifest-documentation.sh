...
# Generate main CHANGELOG.md file
generate_main_changelog() {
    local version="$1"
    local timestamp="$2"
    local release_type="$3"
    local changes_file="$4"
    
    log_info "Generating main CHANGELOG.md..."
    
    local changelog_file="$PROJECT_ROOT/CHANGELOG.md"
    
    # Generate template for main changelog
    local content="# Changelog
...
*Generated by Manifest CLI v${version}*"
    ...
    if [[ -f "$changelog_file" ]]; then
        log_success "Main changelog generated: $changelog_file"
        return 0
    else
        log_error "Failed to generate main changelog"
        return 1
    fi
}
...
update_readme_changelog() {
    ...
    log_info "Updating README changelog section..."
    ...
    local new_entry="### [v${version}] - ${timestamp}
- **${release_type^} Release**: Various improvements and bug fixes
- Enhanced CLI functionality
- Improved error handling
- Better cross-platform compatibility"
    ...
    # Add actual changes if available
    if [[ -f "$changes_file" ]]; then
        local changes=$(cat "$changes_file" | sed 's/\x1b\[[0-9;]*m//g' | head -5)  # Take first 5 lines
        new_entry="${new_entry}
- ${changes}"
    fi
    ...
    while IFS= read -r line; do
        if [[ "$line" == "## 📋 Changelog" ]]; then
            ...
            echo "" >> "$temp_file"
            added_entry=true
        elif [[ "$in_changelog" == true && "$line" == "### "* ]]; then
            echo "$line" >> "$temp_file"
        else
            echo "$line" >> "$temp_file"
        fi
    ...
    if [[ "$added_entry" == true ]]; then
        log_success "README changelog section updated"
        return 0
    else
        log_warning "Could not find changelog section in README"
        return 1
    fi
}
...
generate_documents() {
    ...
    generate_main_changelog "$version" "$timestamp" "$release_type" "$changes_file"
    update_readme_version "$version" "$timestamp"
    update_readme_changelog "$version" "$timestamp" "$release_type" "$changes_file"
