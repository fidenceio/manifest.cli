...
# container; the host's git config is untouched. jq is a test-only dependency
# (the JSON assertions in version_sync.bats etc.); it is ambient on the macOS
# runner image but must be installed explicitly in the minimal alpine image.
docker run --rm \
    -v "$REPO_ROOT:/work" \
    -w /work \
    alpine:3.20 \
    sh -lc 'apk add --no-cache bash git bats parallel yq coreutils jq >/dev/null && git config --system --add safe.directory "*" && bash scripts/run-tests.sh "$@"' \
    sh "$@"
