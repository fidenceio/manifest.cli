...
# Coverage for the undeclared-remote ship-time gate (_fleet_preflight_no_empty_remote).
#
...
# member from the fleet config's declared url where it can, and refuses the
...
#   3. Isolated refuse     — real loader + direct gate call, no declared url
...
# Write a one-member fleet at $SCRATCH/work. $1 is the member's declared url
# (may be empty). The fleet root itself is a git repo, as in production.
#
# The declaration lives in manifest.fleet.config.yaml, not in the roster: §45
# removed the roster's REMOTE_URL column because that file is committed and
# pushed by design, so a credentialed URL in it is published irrecoverably.
...
# Load the real fleet config so get_fleet_service_property resolves the member url,
...
@test "ship fleet -y refuses when a releaseable member has no origin and no declared url" {
    write_one_member_fleet ""
...
@test "ship fleet --local does NOT invoke the empty-remote gate (never pushes)" {
    ...
    [[ "$output" != *"no usable url declared in the fleet config"* ]]
}

@test "gate repairs origin from the declared fleet-config url when it is a valid git URL" {
...
@test "gate refuses (and adds no remote) when no url is declared" {
    write_one_member_fleet ""
