...
@test "init fleet phase 1 TSV uses freshness metadata instead of VERSION column" {
    ...
    ! grep -q '^# Generated by:' "$SCRATCH/manifest.fleet.tsv"
    grep -q '^# Canonical config: manifest.fleet.config.yaml$' "$SCRATCH/manifest.fleet.tsv"
    grep -q $'^# SELECT\tNAME\tPATH\tHAS_GIT\tREMOTE_URL\tBRANCH$' "$SCRATCH/manifest.fleet.tsv"
...
true	rootworkspace	.	true	git@example.com:org/root.git	main
TSV
    local discovered
    ...
    grep -q $'^true\trootworkspace\t.\ttrue\tgit@example.com:org/root.git\tmain$' "$SCRATCH/merged.tsv"
    grep -q $'^true\tsvc\tservices/api\ttrue\tgit@example.com:org/api.git\tmain$' "$SCRATCH/merged.tsv"
}
...
@test "merge_update_tsv keeps empty TSV fields aligned" {
    ...
    [ "$(awk -F '\t' '$2 == "plain" {print $4 "|" $5 "|" $6}' "$SCRATCH/merged.tsv")" = "false||" ]
}
