...
@test "yaml: maps gh-interaction keys (§9.2 rate limit, §9.27(a) CI verdict)" {
    run yaml_path_to_env_var "github.rate_limit_per_hour"
    [ "$status" -eq 0 ]
    [ "$output" = "MANIFEST_CLI_GITHUB_RATE_LIMIT_PER_HOUR" ]

    run env_var_to_yaml_path "MANIFEST_CLI_GITHUB_RATE_LIMIT_PER_HOUR"
    [ "$status" -eq 0 ]
    [ "$output" = "github.rate_limit_per_hour" ]

    run yaml_path_to_env_var "release.require_ci_green"
    [ "$status" -eq 0 ]
    [ "$output" = "MANIFEST_CLI_RELEASE_REQUIRE_CI_GREEN" ]

    run env_var_to_yaml_path "MANIFEST_CLI_RELEASE_REQUIRE_CI_GREEN"
    [ "$status" -eq 0 ]
    [ "$output" = "release.require_ci_green" ]
}
...
@test "yaml: write_full_yaml dumps mapped MANIFEST_CLI_* values as structured YAML" {
    ...
    # Header comments survive the per-key yq writes.
    local header
    header="$(head -2 "$YAML")"
    grep -q "# Manifest CLI Configuration" <<<"$header"
    grep -q "# Generated by manifest-yaml.sh" <<<"$header"
...
@test "yaml: write_full_yaml round-trips through load_yaml_to_env" {
    export MANIFEST_CLI_GIT_TAG_PREFIX="rt-"
    run write_full_yaml "$YAML"
    [ "$status" -eq 0 ]

    # Re-import into a clean env: the dumped file is a valid config layer.
    run env -u MANIFEST_CLI_GIT_TAG_PREFIX bash -c \
        'source "$1/tests/helpers/setup.bash"; load_modules; load_yaml_to_env "$2" >/dev/null; printf "%s" "$MANIFEST_CLI_GIT_TAG_PREFIX"' _ "$TEST_REPO_ROOT" "$YAML"
    [ "$status" -eq 0 ]
    [ "$output" = "rt-" ]
}

@test "yaml: write_full_yaml requires an output path and an existing parent dir" {
    run write_full_yaml ""
    [ "$status" -eq 1 ]
    echo "$output" | grep -q "output file path is required"

    run write_full_yaml "$SCRATCH/missing-dir/out.yaml"
    [ "$status" -eq 1 ]
    echo "$output" | grep -q "parent directory does not exist"
    [ ! -e "$SCRATCH/missing-dir" ]
}
